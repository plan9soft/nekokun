...
namespace orzTech.NekoKun.ProjectEngines.RGSS.Zlib
{
    public class Deflate : ZStream
    {
        ...
        private Deflate(int level)
            : this(level, -1, -1, -1)
        {
        }

        private Deflate(int level, int windowBits)
            : this(level, windowBits, -1, -1)
        {
        }

        private Deflate(int level, int windowBits, int memlevel)
            : this(level, windowBits, memlevel, -1)
        {
        }

        private Deflate(int level, int windowBits, int memlevel, int strategy)
        {
        }

        public static byte[] DeflateString(byte[] zstring)
        {
            Deflate self = new Deflate();
            return self.DeflateStringReal(zstring, 4);
        }

        public byte[] DeflateStringReal(byte[] str, int flush)
        {
            if (flush != 4)
            {
                throw new NotImplementedException("flush can only be FINISH");
            }
            MemoryStream stream = new MemoryStream(str);
            MemoryStream output = new MemoryStream();
            ZDeflateStream stream3 = new ZDeflateStream(output, false);
            int length = str.Length;
            byte[] buffer = new byte[Math.Min(0x1000, length)];
            while (length > 0)
            {
        ...
        internal class ZDeflateStream : DeflateStream
        {
                ...
                : base(output, CompressionMode.Compress, true)
            {
                ...
                byte[] buffer = new byte[] { 0x58, 0x85 };
            ...
            public override IAsyncResult BeginWrite(byte[] array, int offset, int count, AsyncCallback asyncCallback, object asyncState)
            {
                ...
                this._size += count;
                this._crc = UpdateCrc(this._crc, array, offset, count);
                return result;
            }
            ...
            protected override void Dispose(bool disposing)
            {
                ...
                if (disposing && (this._output != null))
                {
                    this._output.WriteByte((byte)(this._crc & 0xff));
                    this._output.WriteByte((byte)((this._crc >> 8) & 0xff));
                    this._output.WriteByte((byte)((this._crc >> 0x10) & 0xff));
                    this._output.WriteByte((byte)((this._crc >> 0x18) & 0xff));
                    if (!this._leaveOpen)
                    {
                        this._output.Close();
                    }
